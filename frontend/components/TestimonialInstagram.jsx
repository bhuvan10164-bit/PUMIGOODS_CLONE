...
const instaImages = Array(6).fill(null).map((_, i) => ({ id: i, fallback: FALLBACK_IMGS[i] }));
...
export default function TestimonialInstagram() {
  ...
  const navigate = (dir) => {
    ...
    setAnimating(true);
    setTimeout(() => { setCurrent(nextIdx); setDirection(null); setAnimating(false); }, 380);
  };
  ...
  return (
    <div style={{ background: "#fff", fontFamily: "Arial, sans-serif" }}>
      ...
      <div className="flex flex-col md:hidden" style={{ padding: "40px 20px 0" }}>
        ...
        <div style={{ position: "relative", paddingBottom: "40px" }}
          ...
          {/* Left arrow */}
          <button onClick={() => navigate("prev")} disabled={current === 0 || animating}
            style={{
              position: "absolute", left: "-8px", top: "50%", transform: "translateY(-50%)",
              background: "transparent", border: "none", cursor: current === 0 ? "default" : "pointer",
              padding: "6px", zIndex: 10, color: "#555",
              opacity: current !== 0 ? 1 : 0, transition: "opacity 0.3s",
            }}>
          ...
          {/* Right arrow */}
          <button onClick={() => navigate("next")} disabled={current === total - 1 || animating}
            style={{
              position: "absolute", right: "-8px", top: "50%", transform: "translateY(-50%)",
              background: "transparent", border: "none", cursor: current === total - 1 ? "default" : "pointer",
              padding: "6px", zIndex: 10, color: "#555",
              opacity: current !== total - 1 ? 1 : 0, transition: "opacity 0.3s",
            }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div style={{ paddingLeft: "24px", paddingRight: "24px" }}>
            ...
            <div style={{ textAlign: "center", marginBottom: "20px" }}>
              <div style={{ fontSize: "11px", fontWeight: "700", letterSpacing: "0.2em", textTransform: "uppercase", color: "#3a9e5f", marginBottom: "6px" }}>
                Go Green Accessories
              </div>
              <div style={{ fontSize: "20px", fontWeight: "700", letterSpacing: "0.07em", textTransform: "uppercase", color: "#1a1a1a", fontFamily: "'Georgia', serif", marginBottom: "8px" }}>
                What They Say About Us
              </div>
              <div style={{ display: "flex", justifyContent: "center" }}>
                <div style={{ width: "36px", height: "3px", background: "#3a9e5f" }} />
              </div>
            </div>
            ...
            <div style={{ overflow: "hidden", position: "relative", minHeight: "160px" }}>
              {animating && (
                <div key={"out-" + current} style={{ position: "absolute", width: "100%", animation: `${outClass} 0.38s ease forwards` }}>
                  <p style={{ fontSize: "15px", color: "#555", lineHeight: "1.75", textAlign: "center", marginBottom: "22px" }}>{t.text}</p>
                  <p style={{ fontSize: "14px", color: "#1a1a1a", textAlign: "center" }}>
                    <strong>{t.author}</strong><span style={{ color: "#888", fontWeight: "400" }}> - {t.role}</span>
                  </p>
                </div>
              )}
              <div key={"in-" + current} style={{ animation: `${inClass} 0.38s ease forwards`, opacity: animating ? 0 : 1 }}>
                <p style={{ fontSize: "15px", color: "#555", lineHeight: "1.75", textAlign: "center", marginBottom: "22px" }}>{t.text}</p>
                <p style={{ fontSize: "14px", color: "#1a1a1a", textAlign: "center" }}>
                  <strong>{t.author}</strong><span style={{ color: "#888", fontWeight: "400" }}> - {t.role}</span>
                </p>
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: "8px", marginTop: "20px" }}>
              {testimonials.map((_, i) => (
                <button key={i} onClick={() => {
                  ...
                  setAnimating(true);
                  setTimeout(() => { setCurrent(i); setDirection(null); setAnimating(false); }, 380);
                }} style={{
        ...
        {/* Instagram grid — full width below testimonial */}
        <div style={{ position: "relative" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gridTemplateRows: "repeat(2, 120px)", gap: "4px" }}>
            {instaImages.map((item) => (
              <div key={item.id} style={{ overflow: "hidden", background: "#eee" }}>
                <img src={item.fallback} alt="Instagram post" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
              </div>
            ))}
          </div>
          {/* Overlay card */}
          <div style={{
            ...
            boxShadow: "0 4px 24px rgba(0,0,0,0.13)", minWidth: "160px", zIndex: 10,
          }}>
            <p style={{ fontSize: "16px", fontWeight: "800", color: "#1a1a1a", letterSpacing: "0.04em", marginBottom: "2px", fontFamily: "'Georgia', serif" }}>INSTAGRAM</p>
            <p style={{ fontSize: "12px", color: "#888", marginBottom: "6px" }}>@pumigoods</p>
            <p style={{ fontSize: "11.5px", color: "#555", lineHeight: "1.5" }}>Curated eco-friendly essentials. 🌿</p>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        style={{ alignItems: "center", justifyContent: "center", padding: "40px 20px", minHeight: "100vh" }}
      >
        <div style={{ width: "100%", maxWidth: "1200px", display: "flex", alignItems: "center" }}>

          {/* LEFT: Testimonial */}
          <div style={{ flex: "0 0 42%", paddingRight: "60px", position: "relative" }}
            onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
            ...
            <button onClick={() => navigate("prev")} disabled={current === 0 || animating}
              ...
              style={{
                ...
                background: "transparent", border: "none", cursor: current === 0 ? "default" : "pointer",
                padding: "6px", zIndex: 10, color: "#555",
                opacity: arrowsVisible && current !== 0 ? 1 : 0, transition: "opacity 0.3s",
            ...
            <button onClick={() => navigate("next")} disabled={current === total - 1 || animating}
              ...
              style={{
                ...
                background: "transparent", border: "none", cursor: current === total - 1 ? "default" : "pointer",
                padding: "6px", zIndex: 10, color: "#555",
                opacity: arrowsVisible && current !== total - 1 ? 1 : 0, transition: "opacity 0.3s",
            ...
            <div style={{ paddingLeft: "28px" }}>
              <div className="text-center">
                <div style={{ fontSize: "11px", fontWeight: "700", letterSpacing: "0.2em", textTransform: "uppercase", color: "#3a9e5f", marginBottom: "10px" }}>
                  Go Green Accessories
                  <div style={{ fontSize: "20px", fontWeight: "700", letterSpacing: "0.07em", textTransform: "uppercase", color: "#1a1a1a", marginBottom: "1px", fontFamily: "'Georgia', serif" }}>
                    What They Say About Us
                  </div>
                  <div className="text-center">
          ...
          <div style={{ flex: "0 0 58%", position: "relative" }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gridTemplateRows: "repeat(2, 200px)", gap: "6px" }}>
              {instaImages.map((item) => (
                <div key={item.id} style={{ overflow: "hidden", background: "#eee" }}>
                  <img src={item.fallback} alt="Instagram post" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                </div>
            ...
            <div style={{
              ...
              <p style={{ fontSize: "20px", fontWeight: "800", color: "#1a1a1a", letterSpacing: "0.04em", marginBottom: "4px", fontFamily: "'Georgia', serif" }}>INSTAGRAM</p>
              <p style={{ fontSize: "13px", color: "#888", marginBottom: "14px" }}>@pumigoods</p>
              <p style={{ fontSize: "13.5px", color: "#555", lineHeight: "1.6" }}>Curated eco-friendly essentials for a greener lifestyle. 🌿</p>
            </div>
          </div>
        </div>
      </div>
